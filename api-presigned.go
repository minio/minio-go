...
func (c *Client) presignURL(ctx context.Context, method string, bucketName string, objectName string, expires time.Duration, reqParams url.Values, hostOverride *string) (u *url.URL, err error) {
	// Input validation.
	if method == "" {
...
// PresignedGetObjectWithHostOverride - Returns a presigned URL to access an object
// data without credentials. URL can have a maximum expiry of
// upto 7days or a minimum of 1sec. Additionally you can override
// a set of response headers using the query parameters.
// This method allows URL Host override.
func (c *Client) PresignedGetObjectWithHostOverride(ctx context.Context, bucketName string, objectName string, expires time.Duration, reqParams url.Values, host string) (u *url.URL, err error) {
	if err = s3utils.CheckValidObjectName(objectName); err != nil {
		return nil, err
	}
	return c.presignURL(ctx, http.MethodGet, bucketName, objectName, expires, reqParams, &host)
}

// PresignedHeadObjectWithHostOverride - Returns a presigned URL to access
// object metadata without credentials. URL can have a maximum expiry
// of upto 7days or a minimum of 1sec. Additionally you can override
// a set of response headers using the query parameters.
// This method allows URL Host override.
func (c *Client) PresignedHeadObjectWithHostOverride(ctx context.Context, bucketName string, objectName string, expires time.Duration, reqParams url.Values, host string) (u *url.URL, err error) {
	if err = s3utils.CheckValidObjectName(objectName); err != nil {
		return nil, err
	}
	return c.presignURL(ctx, http.MethodHead, bucketName, objectName, expires, reqParams, &host)
}

// PresignedPutObjectWithHostOverride - Returns a presigned URL to upload an object
// without credentials. URL can have a maximum expiry of upto 7days
// or a minimum of 1sec.
// This method allows URL Host override.
func (c *Client) PresignedPutObjectWithHostOverride(ctx context.Context, bucketName string, objectName string, expires time.Duration, host string) (u *url.URL, err error) {
	if err = s3utils.CheckValidObjectName(objectName); err != nil {
		return nil, err
	}
	return c.presignURL(ctx, http.MethodPut, bucketName, objectName, expires, nil, &host)
}

// PresignWithHostOverride - returns a presigned URL for any http method of your choice
// along with custom request params. URL can have a maximum expiry of
// upto 7days or a minimum of 1sec.
// This method allows URL Host override.
func (c *Client) PresignWithHostOverride(ctx context.Context, method string, bucketName string, objectName string, expires time.Duration, reqParams url.Values, host string) (u *url.URL, err error) {
	return c.presignURL(ctx, method, bucketName, objectName, expires, reqParams, &host)
}
