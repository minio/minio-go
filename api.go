...
func (c Client) executeMethod(method string, metadata requestMetadata) (res *http.Response, err error) {
	...
	for _ = range c.newRetryTimer(retry, time.Second, time.Second*30, MaxJitter, doneCh) {
		...
		if errResponse.Code == "InvalidRegion" && errResponse.Headers.Get("x-amz-bucket-region") != "" {
			c.bucketLocCache.Set(metadata.bucketName, errResponse.Headers.Get("x-amz-bucket-region"))
			continue // Retry.
		}
