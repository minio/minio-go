...
func testMakeBucketError() {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Minio client creation failed", err).Fatal()
	}
	...
	if err = c.MakeBucket(bucketName, region); err != nil {
		failureLog(testName, function, args, startTime, "", "MakeBucket Failed", err).Fatal()
	}
	if err = c.MakeBucket(bucketName, region); err == nil {
		failureLog(testName, function, args, startTime, "", "Bucket already exists", err).Fatal()
	}
	// Verify valid error response from server.
	if minio.ToErrorResponse(err).Code != "BucketAlreadyExists" &&
		minio.ToErrorResponse(err).Code != "BucketAlreadyOwnedByYou" {
		failureLog(testName, function, args, startTime, "", "Invalid error returned by server", err).Fatal()
	}
	if err = c.RemoveBucket(bucketName); err != nil {
		failureLog(testName, function, args, startTime, "", "Remove bucket failed", err).Fatal()
	}
...
func testPutObjectReadAt() {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Minio client object creation failed", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Make bucket failed", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "PutObject failed", err).Fatal()
	}

	if n != int64(sixtyFiveMiB) {
		failureLog(testName, function, args, startTime, "", "Number of bytes returned by PutObject does not match, expected "+string(sixtyFiveMiB)+" got "+string(n), err).Fatal()
	}

	// Read the data back
	r, err := c.GetObject(bucketName, objectName)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Get Object failed", err).Fatal()
	}

	st, err := r.Stat()
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Stat Object failed", err).Fatal()
	}
	if st.Size != int64(sixtyFiveMiB) {
		failureLog(testName, function, args, startTime, "", "Number of bytes in stat does not match, expected "+string(sixtyFiveMiB)+" got "+string(st.Size), err).Fatal()
	}
	if st.ContentType != objectContentType {
		failureLog(testName, function, args, startTime, "", "Content types don't match", err).Fatal()
	}
	if err := r.Close(); err != nil {
		failureLog(testName, function, args, startTime, "", "Object Close failed", err).Fatal()
	}
	if err := r.Close(); err == nil {
		failureLog(testName, function, args, startTime, "", "Object is already closed, didn't return error on Close", err).Fatal()
	}

	err = c.RemoveObject(bucketName, objectName)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "RemoveObject failed", err).Fatal()
	}
	err = c.RemoveBucket(bucketName)

	if err != nil {
		failureLog(testName, function, args, startTime, "", "RemoveBucket failed", err).Fatal()
	}
...
func testPutObjectWithMetadata() {
	...
	r, err := c.GetObject(bucketName, objectName)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "GetObject failed", err).Fatal()
	}

	st, err := r.Stat()
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Stat failed", err).Fatal()
	}
	if st.Size != int64(sixtyFiveMiB) {
		failureLog(testName, function, args, startTime, "", "Number of bytes returned by PutObject does not match GetObject, expected "+string(sixtyFiveMiB)+" got "+string(st.Size), err).Fatal()
	}
	if st.ContentType != customContentType {
		failureLog(testName, function, args, startTime, "", "ContentType does not match, expected "+customContentType+" got "+st.ContentType, err).Fatal()
	}
	if err := r.Close(); err != nil {
		failureLog(testName, function, args, startTime, "", "Object Close failed", err).Fatal()
	}
	if err := r.Close(); err == nil {
		failureLog(testName, function, args, startTime, "", "Object already closed, should respond with error", err).Fatal()
	}
...
func testPutObjectStreaming() {
	...
	for _, size := range sizes {
		...
		if err != nil {
			failureLog(testName, function, args, startTime, "", "PutObjectStreaming failed", err).Fatal()
		}

		if n != size {
			failureLog(testName, function, args, startTime, "", "Expected upload object size doesn't match with PutObjectStreaming return value", err).Fatal()
		}
	}
...
func testListPartiallyUploaded() {
	...
	go func() {
		...
		for i < 25 {
			...
			if cerr != nil {
				failureLog(testName, function, args, startTime, "", "Copy failed", err).Fatal()
			}
			i++
			r.Seek(0, 0)
		}
	...
	if err == nil {
		failureLog(testName, function, args, startTime, "", "PutObject should fail", err).Fatal()
	}
	if !strings.Contains(err.Error(), "proactively closed to be verified later") {
		failureLog(testName, function, args, startTime, "", "String not found in PutObject output", err).Fatal()
	}
	...
	for multiPartObject := range multiPartObjectCh {
		if multiPartObject.Err != nil {
			failureLog(testName, function, args, startTime, "", "Multipart object error", multiPartObject.Err).Fatal()
		}
	}
...
func testGetObjectSeekEnd() {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "ReadAll failed", err).Fatal()
	}
	...
	if n != int64(thirtyThreeKiB) {
		failureLog(testName, function, args, startTime, "", "Number of bytes read does not match, expected "+string(int64(thirtyThreeKiB))+" got "+string(n), err).Fatal()
	}
	...
	if st.Size != int64(thirtyThreeKiB) {
		failureLog(testName, function, args, startTime, "", "Number of bytes read does not match, expected "+string(int64(thirtyThreeKiB))+" got "+string(st.Size), err).Fatal()
	}

	pos, err := r.Seek(-100, 2)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Object Seek failed", err).Fatal()
	}
	if pos != st.Size-100 {
		failureLog(testName, function, args, startTime, "", "Incorrect position", err).Fatal()
	}
	buf2 := make([]byte, 100)
	m, err := io.ReadFull(r, buf2)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Error reading through io.ReadFull", err).Fatal()
	}
	if m != len(buf2) {
		failureLog(testName, function, args, startTime, "", "Number of bytes dont match, expected "+string(len(buf2))+" got "+string(m), err).Fatal()
	}
	hexBuf1 := fmt.Sprintf("%02x", buf[len(buf)-100:])
	hexBuf2 := fmt.Sprintf("%02x", buf2[:m])
	if hexBuf1 != hexBuf2 {
		failureLog(testName, function, args, startTime, "", "Values at same index dont match", err).Fatal()
	}
	pos, err = r.Seek(-100, 2)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Object Seek failed", err).Fatal()
	...
	if err = r.Close(); err != nil {
		failureLog(testName, function, args, startTime, "", "ObjectClose failed", err).Fatal()
	}
	successLogger(testName, function, args, startTime).Info()
}
...
func testGetObjectClosedTwice() {
	...
	if n != int64(thirtyThreeKiB) {
		failureLog(testName, function, args, startTime, "", "PutObject response doesn't match sent bytes, expected "+string(int64(thirtyThreeKiB))+" got "+string(n), err).Fatal()
	}
	...
	if st.Size != int64(thirtyThreeKiB) {
		failureLog(testName, function, args, startTime, "", "Number of bytes in stat does not match, expected "+string(int64(thirtyThreeKiB))+" got "+string(st.Size), err).Fatal()
	}
	if err := r.Close(); err != nil {
		failureLog(testName, function, args, startTime, "", "Object Close failed", err).Fatal()
	}
	if err := r.Close(); err == nil {
		failureLog(testName, function, args, startTime, "", "Already closed object. No error returned", err).Fatal()
	}
...
func testRemoveMultipleObjects() {
	...
	case r, more := <-errorCh:
		if more {
			failureLog(testName, function, args, startTime, "", "Unexpected error", r.Err).Fatal()
		}
	}
...
func testRemovePartiallyUploaded() {
	...
	if !strings.Contains(err.Error(), "proactively closed to be verified later") {
		failureLog(testName, function, args, startTime, "", "String not found", err).Fatal()
	}
	err = c.RemoveIncompleteUpload(bucketName, objectName)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "RemoveIncompleteUpload failed", err).Fatal()
	}
	err = c.RemoveBucket(bucketName)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "RemoveBucket failed", err).Fatal()
...
func testFPutObjectMultipart() {
	...
	if os.Getenv("MINT_DATA_DIR") == "" {
		...
		if err != nil {
			failureLog(testName, function, args, startTime, "", "TempFile creation failed", err).Fatal()
		}
		// Upload 4 parts to utilize all 3 'workers' in multipart and still have a part to upload.
		...
		err = file.Close()
		if err != nil {
			failureLog(testName, function, args, startTime, "", "File Close failed", err).Fatal()
		}
		fileName = file.Name()
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "FPutObject failed", err).Fatal()
	}
	...
	objInfo, err := r.Stat()
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Unexpected error", err).Fatal()
	}
	if objInfo.Size != int64(totalSize) {
		failureLog(testName, function, args, startTime, "", "Number of bytes does not match, expected "+string(int64(totalSize))+" got "+string(objInfo.Size), err).Fatal()
	}
	if objInfo.ContentType != objectContentType {
		failureLog(testName, function, args, startTime, "", "ContentType doesn't match", err).Fatal()
	}
...
func testFPutObject() {
	...
	if os.Getenv("MINT_DATA_DIR") == "" {
		...
		if _, err = file.Write(buffer); err != nil {
			failureLog(testName, function, args, startTime, "", "File write failed", err).Fatal()
		}
	...
	if n != int64(totalSize) {
		failureLog(testName, function, args, startTime, "", "Number of bytes does not match, expected "+string(totalSize)+", got "+string(n), err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "File open failed", err).Fatal()
	}
	defer srcFile.Close()
	// Add extension to temp file name
	tmpFile, err := os.Create(fName + ".gtar")
	if err != nil {
		failureLog(testName, function, args, startTime, "", "File create failed", err).Fatal()
	}
	defer tmpFile.Close()
	_, err = io.Copy(tmpFile, srcFile)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "File copy failed", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "StatObject failed", err).Fatal()
	}
	if rStandard.ContentType != "application/octet-stream" {
		failureLog(testName, function, args, startTime, "", "ContentType does not match, expected application/octet-stream, got "+rStandard.ContentType, err).Fatal()
	}
	...
	if rGTar.ContentType != "application/x-gtar" {
		failureLog(testName, function, args, startTime, "", "ContentType does not match, expected application/x-gtar, got "+rStandard.ContentType, err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "File remove failed", err).Fatal()
	}
	successLogger(testName, function, args, startTime).Info()
}
...
func testFPutObjectWithContext() {
	...
	if os.Getenv("MINT_DATA_DIR") == "" {
		...
		if _, err = file.Write(buffer); err != nil {
			failureLog(testName, function, args, startTime, "", "File buffer write failed", err).Fatal()
		}
		// Close the file pro-actively for windows.
		err = file.Close()
		if err != nil {
	...
	if err == nil {
		failureLog(testName, function, args, startTime, "", "Request context cancellation failed", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "FPutObjectWithContext failed", err).Fatal()
	}
	if n != int64(totalSize) {
		failureLog(testName, function, args, startTime, "", "Number of bytes does not match, expected "+string(totalSize)+", got "+string(n), err).Fatal()
	}
	...
	err = os.Remove(fName)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Remove file failed", err).Fatal()
	}
	successLogger(testName, function, args, startTime).Info()
...
func testFPutObjectWithContextV2() {
	...
	if os.Getenv("MINT_DATA_DIR") == "" {
		...
		file, err := ioutil.TempFile(os.TempDir(), "FPutObjectWithContextTest")
		if err != nil {
			failureLog(testName, function, args, startTime, "", "Temp file creation failed", err).Fatal()
		...
		var buffer = bytes.Repeat([]byte(string('a')), 1024*1024*1)
		if _, err = file.Write(buffer); err != nil {
			failureLog(testName, function, args, startTime, "", "Write buffer to file failed", err).Fatal()
	...
	_, err = c.FPutObjectWithContext(ctx, bucketName, objectName+"-Shorttimeout", fName, &minio.PutObjectOptions{ContentType: "application/octet-stream"})
	if err == nil {
		failureLog(testName, function, args, startTime, "", "FPutObjectWithContext with short timeout failed", err).Fatal()
	}
	ctx, cancel = context.WithTimeout(context.Background(), 10*time.Minute)
	defer cancel()
	// Perform FPutObjectWithContext with a long timeout. Expect the put object to succeed
	n, err := c.FPutObjectWithContext(ctx, bucketName, objectName+"-Longtimeout", fName, nil)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "FPutObjectWithContext with long timeout failed", err).Fatal()
	}
	if n != int64(totalSize) {
		failureLog(testName, function, args, startTime, "", "Number of bytes does not match:wanted"+string(totalSize)+" got "+string(n), err).Fatal()
	}
...
func testPutObjectWithContext() {
	...
	err = c.MakeBucket(bucketName, "us-east-1")
	if err != nil {
		failureLog(testName, function, args, startTime, "", "MakeBucket call failed", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "PutObjectWithContext with short timeout failed", err).Fatal()
	}
	...
	_, err = c.PutObjectWithContext(ctx, bucketName, objectName, reader, int64(bufSize), &minio.PutObjectOptions{ContentType: "binary/octet-stream"})
	if err != nil {
		failureLog(testName, function, args, startTime, "", "PutObjectWithContext with long timeout failed", err).Fatal()
	}
...
func testGetObjectReadSeekFunctional() {
	...
	if n != int64(thirtyThreeKiB) {
		failureLog(testName, function, args, startTime, "", "Number of bytes does not match, expected "+string(int64(thirtyThreeKiB))+", got "+string(n), err).Fatal()
	}
	...
	if st.Size != int64(thirtyThreeKiB) {
		failureLog(testName, function, args, startTime, "", "Number of bytes does not match, expected "+string(int64(thirtyThreeKiB))+", got "+string(st.Size), err).Fatal()
	}
	...
	cmpData := func(r io.Reader, start, end int) {
		...
		if _, err := io.CopyN(buffer, r, int64(thirtyThreeKiB)); err != nil {
			if err != io.EOF {
				failureLog(testName, function, args, startTime, "", "CopyN failed", err).Fatal()
			}
		}
		if !bytes.Equal(buf[start:end], buffer.Bytes()) {
			failureLog(testName, function, args, startTime, "", "Incorrect read bytes v/s original buffer", err).Fatal()
		}
	}
	...
	for i, testCase := range testCases {
		...
		if testCase.err == seekErr && err == nil {
			failureLog(testName, function, args, startTime, "", "Test "+string(i+1)+", unexpected err value: expected: "+testCase.err.Error()+", found: "+err.Error(), err).Fatal()
		}
		...
		if n != testCase.pos {
			failureLog(testName, function, args, startTime, "", "Test "+string(i+1)+", number of bytes seeked does not match, expected "+string(testCase.pos)+", got "+string(n), err).Fatal()
		}
...
func testGetObjectReadAtFunctional() {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "ReadAt failed", err).Fatal()
	}
	if m != len(buf1) {
		failureLog(testName, function, args, startTime, "", "ReadAt read shorter bytes before reaching EOF, expected "+string(len(buf1))+", got "+string(m), err).Fatal()
	}
	if !bytes.Equal(buf1, buf[offset:offset+512]) {
		failureLog(testName, function, args, startTime, "", "Incorrect read between two ReadAt from same offset", err).Fatal()
	}
	offset += 512
	...
	if m != len(buf2) {
		failureLog(testName, function, args, startTime, "", "ReadAt read shorter bytes before reaching EOF, expected "+string(len(buf2))+", got "+string(m), err).Fatal()
	}
	...
	if m != len(buf3) {
		failureLog(testName, function, args, startTime, "", "ReadAt read shorter bytes before reaching EOF, expected "+string(len(buf3))+", got "+string(m), err).Fatal()
	}
	...
	if m != len(buf4) {
		failureLog(testName, function, args, startTime, "", "ReadAt read shorter bytes before reaching EOF, expected "+string(len(buf4))+", got "+string(m), err).Fatal()
	}
	...
	if m != len(buf5) {
		failureLog(testName, function, args, startTime, "", "ReadAt read shorter bytes before reaching EOF, expected "+string(len(buf5))+", got "+string(m), err).Fatal()
	}
	if !bytes.Equal(buf, buf5) {
		failureLog(testName, function, args, startTime, "", "Incorrect data read in GetObject, than what was previously uploaded", err).Fatal()
	}
...
func testPresignedPostPolicy() {
	...
	if err := policy.SetBucket(""); err == nil {
		failureLog(testName, function, args, startTime, "", "SetBucket did not fail for invalid conditions", err).Fatal()
	}
	if err := policy.SetKey(""); err == nil {
		failureLog(testName, function, args, startTime, "", "SetKey did not fail for invalid conditions", err).Fatal()
	}
	if err := policy.SetKeyStartsWith(""); err == nil {
		failureLog(testName, function, args, startTime, "", "SetKeyStartsWith did not fail for invalid conditions", err).Fatal()
	}
	if err := policy.SetExpires(time.Date(1, time.January, 1, 0, 0, 0, 0, time.UTC)); err == nil {
		failureLog(testName, function, args, startTime, "", "SetExpires did not fail for invalid conditions", err).Fatal()
	}
	if err := policy.SetContentType(""); err == nil {
		failureLog(testName, function, args, startTime, "", "SetContentType did not fail for invalid conditions", err).Fatal()
	}
	if err := policy.SetContentLengthRange(1024*1024, 1024); err == nil {
		failureLog(testName, function, args, startTime, "", "SetContentLengthRange did not fail for invalid conditions", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "PresignedPostPolicy failed", err).Fatal()
	}
...
func testCopyObject() {
	...
	if err == nil {
		failureLog(testName, function, args, startTime, "", "SetModifiedSinceCond did not fail for invalid conditions", err).Fatal()
	}
	err = src.SetUnmodifiedSinceCond(time.Date(1, time.January, 1, 0, 0, 0, 0, time.UTC))
	if err == nil {
		failureLog(testName, function, args, startTime, "", "SetUnmodifiedSinceCond did not fail for invalid conditions", err).Fatal()
	}
	err = src.SetMatchETagCond("")
	if err == nil {
		failureLog(testName, function, args, startTime, "", "SetMatchETagCond did not fail for invalid conditions", err).Fatal()
	}
	err = src.SetMatchETagExceptCond("")
	if err == nil {
		failureLog(testName, function, args, startTime, "", "SetMatchETagExceptCond did not fail for invalid conditions", err).Fatal()
	}

	err = src.SetModifiedSinceCond(time.Date(2014, time.April, 0, 0, 0, 0, 0, time.UTC))
	if err != nil {
		failureLog(testName, function, args, startTime, "", "SetModifiedSinceCond failed", err).Fatal()
	}
	err = src.SetMatchETagCond(objInfo.ETag)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "SetMatchETagCond failed", err).Fatal()
	}
	args["src"] = src
	...
	args["dst"] = dst
	if err != nil {
		failureLog(testName, function, args, startTime, "", "NewDestinationInfo failed", err).Fatal()
	}

	// Perform the Copy
	err = c.CopyObject(dst, src)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "CopyObject failed", err).Fatal()
	}
	...
	if objInfo.Size != objInfoCopy.Size {
		failureLog(testName, function, args, startTime, "", "Number of bytes does not match, expected "+string(objInfoCopy.Size)+", got "+string(objInfo.Size), err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "SetUnmodifiedSinceCond failed", err).Fatal()
	}
	err = src.SetMatchETagExceptCond(objInfo.ETag)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "SetMatchETagExceptCond failed", err).Fatal()
	}

	// Perform the Copy which should fail
	err = c.CopyObject(dst, src)
	if err == nil {
		failureLog(testName, function, args, startTime, "", "CopyObject did not fail for invalid conditions", err).Fatal()
	}
...
func testEncryptionPutGet() {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "DecodeString for symmetric Key generation failed", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "NewAsymmetricKey for symmetric Key generation failed", err).Fatal()
	}
	...
	for i, testCase := range testCases {
		...
		if err != nil {
			failureLog(testName, function, args, startTime, "", "NewCBCSecureMaterials failed", err).Fatal()
		}
		...
		if err != nil {
			failureLog(testName, function, args, startTime, "", "PutEncryptedObject failed", err).Fatal()
		}

		// Read the data back
		r, err := c.GetEncryptedObject(bucketName, objectName, cbcMaterials)
		if err != nil {
			failureLog(testName, function, args, startTime, "", "GetEncryptedObject failed", err).Fatal()
		}
		defer r.Close()
		...
		if _, err = io.Copy(recvBuffer, r); err != nil {
			failureLog(testName, function, args, startTime, "", "Test "+string(i+1)+", error: "+err.Error(), err).Fatal()
		}
		if recvBuffer.Len() != len(testCase.buf) {
			failureLog(testName, function, args, startTime, "", "Test "+string(i+1)+", Number of bytes of received object does not match, expected "+string(len(testCase.buf))+", got "+string(recvBuffer.Len()), err).Fatal()
		}
		if !bytes.Equal(testCase.buf, recvBuffer.Bytes()) {
			failureLog(testName, function, args, startTime, "", "Test "+string(i+1)+", Encrypted sent is not equal to decrypted, expected "+string(testCase.buf)+", got "+string(recvBuffer.Bytes()), err).Fatal()
		}

		// Remove test object
		err = c.RemoveObject(bucketName, objectName)
		if err != nil {
			failureLog(testName, function, args, startTime, "", "Test "+string(i+1)+", RemoveObject failed with: "+err.Error(), err).Fatal()
		}
		successLogger(testName, function, args, startTime).Info()
...
func testBucketNotification() {
	...
	if len(bNotification.TopicConfigs) != 1 {
		failureLog(testName, function, args, startTime, "", "Duplicate entry added", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "SetBucketNotification failed", err).Fatal()
	}

	bNotification, err = c.GetBucketNotification(bucketName)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "GetBucketNotification failed", err).Fatal()
	}

	if len(bNotification.TopicConfigs) != 1 {
		failureLog(testName, function, args, startTime, "", "Topic config is empty", err).Fatal()
	}

	if bNotification.TopicConfigs[0].Filter.S3Key.FilterRules[0].Value != "jpg" {
		failureLog(testName, function, args, startTime, "", "Couldn't get the suffix", err).Fatal()
	}

	err = c.RemoveAllBucketNotification(bucketName)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "RemoveAllBucketNotification failed", err).Fatal()
	}
	successLogger(testName, function, args, startTime).Info()
}
...
func testFunctional() {
	...
	if err != nil {
		failureLog(testName, function, nil, startTime, "", "Minio client object creation failed", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "File creation failed", err).Fatal()
	}
	for i := 0; i < 3; i++ {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "BucketExists failed", err).Fatal()
	}
	if !exists {
		failureLog(testName, function, args, startTime, "", "Could not find the bucket", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "GetBucketPolicy failed", err).Fatal()
	}
	if policyAccess != "none" {
		failureLog(testName, function, args, startTime, "", "policy should be set to none", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "SetBucketPolicy failed", err).Fatal()
	}
	...
	if policyAccess != "readonly" {
		failureLog(testName, function, args, startTime, "", "policy should be set to readonly", err).Fatal()
	}
	...
	if policyAccess != "writeonly" {
		failureLog(testName, function, args, startTime, "", "policy should be set to writeonly", err).Fatal()
	}
	...
	if policyAccess != "readwrite" {
		failureLog(testName, function, args, startTime, "", "policy should be set to readwrite", err).Fatal()
	}
	// List all buckets.
	...
	if len(buckets) == 0 {
		failureLog(testName, function, args, startTime, "", "Found bucket list to be empty", err).Fatal()
	}
	if err != nil {
		failureLog(testName, function, args, startTime, "", "ListBuckets failed", err).Fatal()
	}
	...
	if !bucketFound {
		failureLog(testName, function, args, startTime, "", "Bucket: "+bucketName+" not found", err).Fatal()
	}
	...
	if n != int64(len(buf)) {
		failureLog(testName, function, args, startTime, "", "Length doesn't match, expected "+string(int64(len(buf)))+" got "+string(n), err).Fatal()
	}
	...
	if !objFound {
		failureLog(testName, function, args, startTime, "", "Object "+objectName+" not found", err).Fatal()
	}
	...
	if !incompObjNotFound {
		failureLog(testName, function, args, startTime, "", "Unexpected dangling incomplete upload found", err).Fatal()
	}
	...
	if !bytes.Equal(newReadBytes, buf) {
		failureLog(testName, function, args, startTime, "", "GetObject bytes mismatch", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "FGetObject failed", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "PresignedHeadObject failed", err).Fatal()
	}
	// Verify if presigned url works.
	resp, err := http.Head(presignedHeadURL.String())
	if err != nil {
		failureLog(testName, function, args, startTime, "", "PresignedHeadObject response incorrect", err).Fatal()
	}
	if resp.StatusCode != http.StatusOK {
		failureLog(testName, function, args, startTime, "", "PresignedHeadObject response incorrect, status "+string(resp.StatusCode), err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "PresignedGetObject failed", err).Fatal()
	}

	// Verify if presigned url works.
	resp, err = http.Get(presignedGetURL.String())
	if err != nil {
		failureLog(testName, function, args, startTime, "", "PresignedGetObject response incorrect", err).Fatal()
	}
	if resp.StatusCode != http.StatusOK {
		failureLog(testName, function, args, startTime, "", "PresignedGetObject response incorrect, status "+string(resp.StatusCode), err).Fatal()
	}
	...
	if !bytes.Equal(newPresignedBytes, buf) {
		failureLog(testName, function, args, startTime, "", "Bytes mismatch for presigned GET URL", err).Fatal()
	}
	if resp.Header.Get("Content-Disposition") != "attachment; filename=\"test.txt\"" {
		failureLog(testName, function, args, startTime, "", "wrong Content-Disposition received "+string(resp.Header.Get("Content-Disposition")), err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "PresignedPutObject failed", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Couldn't make HTTP request with PresignedPutObject URL", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "GetObject after PresignedPutObject failed", err).Fatal()
	}

	newReadBytes, err = ioutil.ReadAll(newReader)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "ReadAll after GetObject failed", err).Fatal()
	}

	if !bytes.Equal(newReadBytes, buf) {
		failureLog(testName, function, args, startTime, "", "Bytes mismatch", err).Fatal()
	}
	...
	err = c.RemoveBucket(bucketName)
	if err == nil {
		failureLog(testName, function, args, startTime, "", "RemoveBucket did not fail for invalid bucket name", err).Fatal()
	}
...
func testGetObjectObjectModified() {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Failed to upload "+objectName+", to bucket "+bucketName, err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Failed to GetObject "+objectName+", from bucket "+bucketName, err).Fatal()
	}
	defer reader.Close()
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Failed to read object "+objectName+", from bucket "+bucketName+" at an offset", err).Fatal()
	}
	...
	if err.Error() != expectedError {
		failureLog(testName, function, args, startTime, "", "Expected Stat to fail with error "+expectedError+", but received "+err.Error(), err).Fatal()
	}

	// Read again only to find object contents have been modified since last read.
	_, err = reader.ReadAt(b, int64(n))
	if err.Error() != expectedError {
		failureLog(testName, function, args, startTime, "", "Expected ReadAt to fail with error "+expectedError+", but received "+err.Error(), err).Fatal()
	}
	successLogger(testName, function, args, startTime).Info()
}
...
func testPutObjectUploadSeekedObject() {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "TempFile create failed", err).Fatal()
	}
	...
	if _, err = tempfile.Write(data); err != nil {
		failureLog(testName, function, args, startTime, "", "TempFile write failed", err).Fatal()
	}
	...
	if _, err := tempfile.Seek(int64(offset), 0); err != nil {
		failureLog(testName, function, args, startTime, "", "TempFile seek failed", err).Fatal()
	}
	...
	if n != int64(length-offset) {
		failureLog(testName, function, args, startTime, "", "Invalid length returned, expected "+string(int64(length-offset))+" got "+string(n), err).Fatal()
	}
	tempfile.Close()
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Seek failed", err).Fatal()
	}
	if n != int64(offset) {
		failureLog(testName, function, args, startTime, "", "Invalid offset returned, expected "+string(int64(offset))+" got "+string(n), err).Fatal()
	}
	...
	if n != int64(length-offset) {
		failureLog(testName, function, args, startTime, "", "Invalid offset returned, expected "+string(int64(length-offset))+" got "+string(n), err).Fatal()
	}
...
func testMakeBucketErrorV2() {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Minio v2 client object creation failed", err).Fatal()
	}
	...
	if err = c.MakeBucket(bucketName, region); err == nil {
		failureLog(testName, function, args, startTime, "", "MakeBucket did not fail for existing bucket name", err).Fatal()
	}
	// Verify valid error response from server.
	if minio.ToErrorResponse(err).Code != "BucketAlreadyExists" &&
		minio.ToErrorResponse(err).Code != "BucketAlreadyOwnedByYou" {
...
func testGetObjectClosedTwiceV2() {
	...
	if n != int64(thirtyThreeKiB) {
		failureLog(testName, function, args, startTime, "", "Number of bytes does not match, expected "+string(thirtyThreeKiB)+" got "+string(n), err).Fatal()
	}
	...
	if st.Size != int64(thirtyThreeKiB) {
		failureLog(testName, function, args, startTime, "", "Number of bytes does not match, expected "+string(thirtyThreeKiB)+" got "+string(st.Size), err).Fatal()
	}
	if err := r.Close(); err != nil {
		failureLog(testName, function, args, startTime, "", "Stat failed", err).Fatal()
	}
	if err := r.Close(); err == nil {
		failureLog(testName, function, args, startTime, "", "Object is already closed, should return error", err).Fatal()
	}
...
func testRemovePartiallyUploadedV2() {
	...
	go func() {
		...
		for i < 25 {
			_, cerr := io.CopyN(writer, r, 128*1024)
			if cerr != nil {
				failureLog(testName, function, args, startTime, "", "Copy failed", cerr).Fatal()
			}
			i++
			r.Seek(0, 0)
		}
	...
	if err.Error() != "proactively closed to be verified later" {
		failureLog(testName, function, args, startTime, "", "Unexpected error, expected : proactively closed to be verified later", err).Fatal()
	}
	err = c.RemoveIncompleteUpload(bucketName, objectName)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "RemoveIncompleteUpload failed", err).Fatal()
...
func testFPutObjectV2() {
	...
	if n != int64(11*1024*1024) {
		failureLog(testName, function, args, startTime, "", "Number of bytes does not match, expected "+string(int64(11*1024*1024))+" got "+string(n), err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Rename failed", err).Fatal()
	}
	...
	if rStandard.ContentType != "application/octet-stream" {
		failureLog(testName, function, args, startTime, "", "Content-Type headers mismatched, expected: application/octet-stream , got "+rStandard.ContentType, err).Fatal()
	}
	...
	if rOctet.ContentType != "application/octet-stream" {
		failureLog(testName, function, args, startTime, "", "Content-Type headers mismatched, expected: application/octet-stream , got "+rOctet.ContentType, err).Fatal()
	}
	...
	if rGTar.ContentType != "application/x-gtar" {
		failureLog(testName, function, args, startTime, "", "Content-Type headers mismatched, expected: application/x-gtar , got "+rGTar.ContentType, err).Fatal()
	}
...
func testGetObjectReadSeekFunctionalV2() {
	...
	if n != offset {
		failureLog(testName, function, args, startTime, "", "Number of seeked bytes does not match, expected "+string(offset)+" got "+string(n), err).Fatal()
	}
	n, err = r.Seek(0, 1)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Seek failed", err).Fatal()
	...
	if err == nil {
		failureLog(testName, function, args, startTime, "", "Seek on positive offset for whence '2' should error out", err).Fatal()
	}
	...
	if n != st.Size-offset {
		failureLog(testName, function, args, startTime, "", "Number of seeked bytes does not match, expected "+string(st.Size-offset)+" got "+string(n), err).Fatal()
	}
	...
	if n != (offset - 1) {
		failureLog(testName, function, args, startTime, "", "Number of seeked bytes does not match, expected "+string(offset-1)+" got "+string(n), err).Fatal()
	}
...
func testComposeObjectErrorCasesWrapper(c *minio.Client) {
	...
	if err := c.ComposeObject(dst, srcSlice); err == nil {
		failureLog(testName, function, args, startTime, "", "Expected error in ComposeObject", err).Fatal()
	} else if err.Error() != "There must be as least one and up to 10000 source objects." {
		failureLog(testName, function, args, startTime, "", "Got unexpected error", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Setting NewSourceInfo failed", err).Fatal()
	}
	// 3. ComposeObject call should fail.
	if err := c.ComposeObject(dst, []minio.SourceInfo{badSrc}); err == nil {
		failureLog(testName, function, args, startTime, "", "ComposeObject expected to fail", err).Fatal()
	} else if !strings.Contains(err.Error(), "has invalid segment-to-copy") {
		failureLog(testName, function, args, startTime, "", "Got invalid error", err).Fatal()
	}
	successLogger(testName, function, args, startTime).Info()
}
...
func testComposeMultipleSources(c *minio.Client) {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "SetRange failed", err).Fatal()
	}
	args["sources"] = srcs
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "ComposeObject failed", err).Fatal()
	}
	...
	if objProps.Size != 9*srcSize+1 {
		failureLog(testName, function, args, startTime, "", "Size mismatched! Expected "+string(10000*srcSize)+" got "+string(objProps.Size), err).Fatal()
	}
	successLogger(testName, function, args, startTime).Info()
}
...
func testEncryptedCopyObjectWrapper(c *minio.Client) {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "PutObject call failed", err).Fatal()
	}
	...
	if !bytes.Equal(decBytes, buf) {
		failureLog(testName, function, args, startTime, "", "Downloaded object mismatched for encrypted object", err).Fatal()
	}
	successLogger(testName, function, args, startTime).Info()
}
...
func testUserMetadataCopyingWrapper(c *minio.Client) {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "PutObjectWithMetadata failed", err).Fatal()
	}
	if !reflect.DeepEqual(metadata, fetchMeta("srcObject")) {
		failureLog(testName, function, args, startTime, "", "Metadata match failed", err).Fatal()
	}
...
func testUserMetadataCopyingV2() {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Minio client v2 object creation failed", err).Fatal()
	}
...
func testPutObjectNoLengthV2() {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "PutObjectWithSize failed", err).Fatal()
	}
	if n != int64(sixtyFiveMiB) {
		failureLog(testName, function, args, startTime, "", "Expected upload object size "+string(sixtyFiveMiB)+" got "+string(n), err).Fatal()
	}
...
func testPutObjectsUnknownV2() {
	...
	for i := 1; i <= 4; i++ {
		...
		if n != int64(4) {
			failureLog(testName, function, args, startTime, "", "Expected upload object size "+string(4)+" got "+string(n), err).Fatal()
		}
...
func testPutObject0ByteV2() {
	...
	if n != 0 {
		failureLog(testName, function, args, startTime, "", "Expected upload object size 0 but got "+string(n), err).Fatal()
	}
...
func testFunctionalV2() {
	...
	if !exists {
		failureLog(testName, function, args, startTime, "", "Could not find existing bucket "+bucketName, err).Fatal()
	}
	...
	buckets, err := c.ListBuckets()
	if len(buckets) == 0 {
		failureLog(testName, function, args, startTime, "", "List buckets cannot be empty", err).Fatal()
	}
	if err != nil {
		failureLog(testName, function, args, startTime, "", "ListBuckets failed", err).Fatal()
	}
	...
	if n != int64(len(buf)) {
		failureLog(testName, function, args, startTime, "", "Expected uploaded object length "+string(len(buf))+" got "+string(n), err).Fatal()
	}
	...
	if !objFound {
		failureLog(testName, function, args, startTime, "", "Could not find existing object "+objectName, err).Fatal()
	}
	...
	resp, err := http.Head(presignedHeadURL.String())
	if err != nil {
		failureLog(testName, function, args, startTime, "", "PresignedHeadObject URL head request failed", err).Fatal()
	}
	if resp.StatusCode != http.StatusOK {
		failureLog(testName, function, args, startTime, "", "PresignedHeadObject URL returns status "+string(resp.StatusCode), err).Fatal()
	}
	if resp.Header.Get("ETag") == "" {
		failureLog(testName, function, args, startTime, "", "Got empty ETag", err).Fatal()
	}
	resp.Body.Close()
	...
	resp, err = http.Get(presignedGetURL.String())
	if err != nil {
		failureLog(testName, function, args, startTime, "", "PresignedGetObject URL GET request failed", err).Fatal()
	}
	if resp.StatusCode != http.StatusOK {
		failureLog(testName, function, args, startTime, "", "PresignedGetObject URL returns status "+string(resp.StatusCode), err).Fatal()
	}
	newPresignedBytes, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "ReadAll failed", err).Fatal()
	...
	if resp.Header.Get("Content-Disposition") != "attachment; filename=\"test.txt\"" {
		failureLog(testName, function, args, startTime, "", "wrong Content-Disposition received ", err).Fatal()
	}
	...
	req, err := http.NewRequest("PUT", presignedPutURL.String(), bytes.NewReader(buf))
	if err != nil {
		failureLog(testName, function, args, startTime, "", "HTTP request to PresignedPutObject URL failed", err).Fatal()
	}
	httpClient := &http.Client{
		// Setting a sensible time out of 30secs to wait for response
		// headers. Request is pro-actively cancelled after 30secs
	...
	err = c.RemoveBucket(bucketName)
	if err == nil {
		failureLog(testName, function, args, startTime, "", "RemoveBucket should fail as bucket does not exist", err).Fatal()
	}
	if err.Error() != "The specified bucket does not exist" {
		failureLog(testName, function, args, startTime, "", "RemoveBucket failed with wrong error message", err).Fatal()
	}
	if err = os.Remove(fileName); err != nil {
		failureLog(testName, function, args, startTime, "", "File remove failed", err).Fatal()
	}
	if err = os.Remove(fileName + "-f"); err != nil {
		failureLog(testName, function, args, startTime, "", "File removes failed", err).Fatal()
	}
	successLogger(testName, function, args, startTime).Info()
}
...
func testGetObjectWithContext() {
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "Minio client v4 object creation failed", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "GetObjectWithContext failed - request timeout not honored", err).Fatal()
	...
	r, err = c.GetObjectWithContext(ctx, bucketName, objectName)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "GetObjectWithContext failed", err).Fatal()

	}

	st, err := r.Stat()
	if err != nil {
		failureLog(testName, function, args, startTime, "", "object Stat call failed", err).Fatal()
	}
	if st.Size != int64(bufSize) {
		failureLog(testName, function, args, startTime, "", "Number of bytes in stat does not match: want "+string(bufSize)+", got"+string(st.Size), err).Fatal()
	}
	if err := r.Close(); err != nil {
		failureLog(testName, function, args, startTime, "", "object Close() call failed", err).Fatal()
	}

	err = c.RemoveObject(bucketName, objectName)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "RemoveObject call failed", err).Fatal()
	}
	err = c.RemoveBucket(bucketName)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "RemoveBucket call failed", err).Fatal()
	}
	successLogger(testName, function, args, startTime).Info()
...
func testFGetObjectWithContext() {
	...
	if err == nil {
		failureLog(testName, function, args, startTime, "", "FGetObjectWithContext with short timeout failed", err).Fatal()
	}
	...
	if err != nil {
		failureLog(testName, function, args, startTime, "", "FGetObjectWithContext with long timeout failed", err).Fatal()
	}
...
func testGetObjectWithContextV2() {
	...
	r, err := c.GetObjectWithContext(ctx, bucketName, objectName)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "GetObjectWithContext failed due to non-cancellation upon short timeout", err).Fatal()
	...
	r, err = c.GetObjectWithContext(ctx, bucketName, objectName)
	if err != nil {
		failureLog(testName, function, args, startTime, "", "GetObjectWithContext failed due to non-cancellation upon long timeout", err).Fatal()
	...
	if st.Size != int64(bufSize) {
		failureLog(testName, function, args, startTime, "", "Number of bytes in stat does not match, expected "+string(bufSize)+" got "+string(st.Size), err).Fatal()
	}
	if err := r.Close(); err != nil {
		failureLog(testName, function, args, startTime, "", " object Close() call failed", err).Fatal()
	}
...
func testFGetObjectWithContextV2() {
	...
	err = c.FGetObjectWithContext(ctx, bucketName, objectName, fileName+"-f")
	if err == nil {
		failureLog(testName, function, args, startTime, "", "FGetObjectWithContext call with short request timeout failed", err).Fatal()
	...
	err = c.FGetObjectWithContext(ctx, bucketName, objectName, fileName+"-fcontext")
	if err != nil {
		failureLog(testName, function, args, startTime, "", "FGetObjectWithContext call with long request timeout failed", err).Fatal()
	}
